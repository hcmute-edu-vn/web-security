		...
		<c:choose>
			...
			<c:otherwise>
				<a class="page-item btn-submit"
					href="<%=context%>/system/users/?role_id=${roleId}&current_page=${currentPage - 1}&page_size=${pageSize}"><span>Previous</span></a>
			</c:otherwise>
		</c:choose>
			...
			<c:choose>
				<c:when test="${item == currentPage}">
					<a class="page-item btn-submit active"
						href="<%=context%>/system/users/?role_id=${roleId}&current_page=${item}&page_size=${pageSize}"><span>${item}</span></a>
				</c:when>
				<c:otherwise>
					<a class="page-item btn-submit"
						href="<%=context%>/system/users/?role_id=${roleId}&current_page=${item}&page_size=${pageSize}"><span>${item}</span></a>
